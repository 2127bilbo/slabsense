...
function cfgFrom(){
  ...
  if(Q.mode==="queue"&&queueGrade){c.grade=queueGrade.grade;c.gradeWord=queueGrade.gradeWord;}
  return c;
}
...
function render(){
  ...
  var settingsForBuild=Q.mode==="queue"?Object.assign({},S,{useToken:false}):S;
  return SlabLabel.payload(input,settingsForBuild).then(function(p){
    if(my!==seq)return;
    var b=SlabLabel.build(input,settingsForBuild,p.url,p.alnum);
...
function initAuth(){
  ...
  return fetch("/api/slabs/config").then(function(r){return r.json();}).then(function(c){
    ...
    function apply(session){
      ...
      if(session){qEl("authWho").textContent=session.user.email||"";qEl("authMsg").textContent="";}
...
window.SlabStudio={
  ...
  queue:{setRows:function(rows){Q.rows=rows;setMode("queue");renderQueueList();},select:selectQueueRow,mode:function(){return Q.mode;}},
  getEngravePayload:engravePayload
};

SlabLabel.ready.then(function(){load();return initAuth();}).then(function(){return render();}).then(readyResolve,function(e){document.getElementById("payload").textContent="Label engine failed to load: "+e.message;});
})();
