...
def tile_side(args) -> tuple[list[dict], int]:
    """Cut one side-view's tiles. args = (image_path, cert, side, view, grade_label, boxes_frac_json, out_dir, seed, neg_per_side).
    boxes_frac_json: JSON list of [label, x, y, w, h] fractions. Returns (index rows, failed) — rows'
    tile_path is relative to cache_dir; failed is 1 (rows always []) if the image could not be
    opened/decoded/cropped, else 0. Errors are caught here (not around the Pool call) so one bad
    cached image can't abort the whole run in either the serial or Pool path."""
    ...
    try:
        ...
        print(f"tile: failed {image_path}: {e}", file=sys.stderr, flush=True)
        return [], 1
    return rows, 0
...
def build_tile_index(cache_dir: Path, split: str, sides: pd.DataFrame, boxes: pd.DataFrame,
    ...
    if workers <= 1:
        ...
        with Pool(workers) as pool:
            for r, f in pool.imap_unordered(tile_side, jobs, chunksize=4):
                rows += r
                failed += f
                if progress: progress(len(rows))
...
def _run_tile(cfg, split, limit, workers, seed, neg_per_side):
    ...
    print(f"{split}: {len(df)} tiles ({int((df.n_boxes > 0).sum())} positive, {df.n_boxes.sum()} boxes) "
          f"from {len(sides) - df.attrs['skipped_sides'] - df.attrs['failed_sides']} side-views, "
          f"{df.attrs['skipped_sides']} not cached, {df.attrs['failed_sides']} failed, "
          f"{time.time() - t0:.0f}s")
