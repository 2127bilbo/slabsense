...
function saveCheckpoint(processed, hashes) {
  fs.writeFileSync(CHECKPOINT_FILE, JSON.stringify({ processed, hashes }, null, 2));
}
...
async function buildHashDatabase() {
  ...
  let checkpoint = { processed: 0, hashes: [] };
  if (resumeMode) {
  ...
  for (const card of cardsToProcess) {
    ...
    try {
      const hash = await computePHash(imageUrl);
      ...
      if (processed % CHECKPOINT_INTERVAL === 0) {
        saveCheckpoint(processed, hashes);
      }
  ...
  console.log('='.repeat(50));
  console.log(`Total cards: ${hashes.length}`);
