...
| `python -m tagdataset build` | Write training tables to `data/dataset/`; splits are frozen across rebuilds and read/written at `splits/splits.parquet` (tracked in git), with a copy in `data/dataset/splits.parquet`. `--splits path.parquet` points at a different authoritative split file; existing assignments in it are never changed. |
| `python -m tagdataset stats` | Print the dataset health report (reads `data/dataset/` by default; `--out` for another directory) with the store-joined `ding crops not in files table` count filled in. `--save path.txt` also writes the report text to that path. |
